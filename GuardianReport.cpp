// GuardianReport.cpp — Proceso secundario que cuenta reportes y ordena KILL
// Compila: g++ -std=c++17 -Wall -Wextra -O2 GuardianReport.cpp -o guardian
#include <bits/stdc++.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(){
    ...
    if(fd_in  < 0){ perror("[guardian] open from_srv"); return 1; }
    int fd_out = open(to_srv.c_str(),   O_WRONLY);
    if(fd_out < 0){ perror("[guardian] open to_srv");   return 1; }
    ...
    while(true){
        ...
        if(n > 0){ buf.append(tmp, tmp+n); flush_lines(buf); }
        else if(n == 0){
            close(fd_in);
            fd_in = open(from_srv.c_str(), O_RDONLY);
            if(fd_in < 0){ perror("[guardian] reopen from_srv"); break; }
        }else{
